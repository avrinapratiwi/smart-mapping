  ...
  void _login() {
    ...
    if (_usernameController.text.isEmpty || _passwordController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Tolong isi username dan password!'), backgroundColor: Colors.red),
      );
      return;
    }

    // Perintah kepada BLoC: Tolong proses login dengan data ini!
    context.read<AuthBloc>().add(
      LoginRequested(
        username: _usernameController.text, 
        password: _passwordController.text, 
        role: _selectedRole ?? 'Petugas'
      )
    );
  }
      ...
      body: BlocListener<AuthBloc, AuthState>(
        listener: (context, state) {
          if (state is AuthAuthenticated) {
            // Berhasil login? Langsung terbang ke Dashboard!
            context.go('/dashboard');
          } else if (state is AuthError) {
            // Gagal? Tampilkan pemberitahuan merah (SNACKBAR)
            ScaffoldMessenger.of(context).showSnackBar(
                      ...
                      ShaderMask(
                        ...
                        child: const Text(
                          ...
                          style: TextStyle(
                            ...
                            color: Colors
                                .white, // Dibutuhkan agar ShaderMask bekerja
                            letterSpacing: 4.0,
                          ),
                        ),
                      ),
                          ...
                          LayoutBuilder(
                            builder: (context, constraints) {
                              return DropdownMenu<String>(
                                ...
                                dropdownMenuEntries:
                                    ['Petugas', 'Admin kab/kota', 'Super Admin']
                                        .map(
